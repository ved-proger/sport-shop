 ...
 */
    ...
    (() => {
        ...
        function rangeInit() {
            ...
            if (priceRange) {
                priceRange.getAttribute("data-from");
                priceRange.getAttribute("data-to");
        ...
        function initSliders() {
            if (document.querySelector(".mainslider")) new core(".mainslider", {
                ...
                pagination: {
                    ...
                    renderBullet: function(index, className) {
                        return '<button class="' + className + '" style="background-image: url(' + document.querySelectorAll(".mainslider__slide img")[index].src + ')"><span class="mainslider__index">' + (index + 1) + "</span></button>";
                    }
                },
                on: {}
            });
            ...
            if (document.querySelector(".partners-slider__body")) new core(".partners-slider__body", {
                modules: [ Navigation, Pagination ],
                observer: true,
                observeParents: true,
                slidesPerView: 5,
            ...
            if (document.querySelector(".mainSlider-product")) {
                if (document.querySelector(".mainSlider-product__slide")) {
                    let slides = document.querySelectorAll(".mainSlider-product__slide");
                    let subSliderWrap = document.querySelector(".subSlider-product__wrapper");
                    let subSlides = "";
                    for (let i = 0; i < slides.length; i++) {
                        const slide = slides[i];
                        const srcImg = slide.querySelector(".mainSlider-product__img > img").getAttribute("src");
                        subSlides += `<div class="subSlider-product__slide"><button class="thumbnail-button" type="button"><img src="${srcImg}"></button></div>`;
                    }
                    subSliderWrap.innerHTML = subSlides;
                }
                const productSubSlider = new core(".subSlider-product", {
                    modules: [ Navigation, Pagination ],
                    observer: true,
                    observeParents: true,
                    wrapperClass: "subSlider-product__wrapper",
                    slideClass: "subSlider-product__slide",
                    spaceBetween: 10,
                    slidesPerView: 4,
                    freeMode: true,
                    watchSlidesProgress: true
                });
                new core(".mainSlider-product", {
                    modules: [ Navigation, Pagination, Thumb ],
                    observer: true,
                    observeParents: true,
                    wrapperClass: "mainSlider-product__wrapper",
                    slideClass: "mainSlider-product__slide",
                    direction: "horizontal",
                    thumbs: {
        ...
        da.init();
        let phoneToggle = document.querySelector(".icon-phone-toggle");
        phoneToggle.addEventListener("click", (function() {
            this.parentElement.classList.toggle("open-contacts");
        }));
        document.addEventListener("click", (function(elem) {
            if (!elem.target.closest(".contacts-header")) document.querySelector(".contacts-header").classList.remove("open-contacts");
        }));
        let catalogIcon = document.querySelector(".catalog__icon-menu");
        let menuCatalog = document.querySelector(".menu-catalog");
        catalogIcon.addEventListener("click", (function() {
            document.querySelector(".catalog").classList.toggle("catalog-open");
            _slideToggle(menuCatalog);
        }));
        if (isMobile.any()) {
            let itemParentsLink = document.querySelectorAll(".menu-catalog__item_parent > .menu-catalog__link-parent");
            document.querySelectorAll(".subcatalog__item");
            for (let i = 0; i < itemParentsLink.length; i++) itemParentsLink[i].addEventListener("click", (function(e) {
                e.preventDefault();
                this.parentElement.classList.toggle("subcatalog__item_active");
            }));
        } else {
            let itemParents = document.querySelectorAll(".menu-catalog__item_parent");
            document.querySelectorAll(".subcatalog__item");
            for (let item of itemParents) {
                item.addEventListener("mouseenter", (function(e) {
                    e.preventDefault();
                    item.classList.add("subcatalog__item_active");
                }));
                item.addEventListener("mouseleave", (function(e) {
                    e.preventDefault();
                    item.classList.remove("subcatalog__item_active");
                }));
            }
            let catalogLinks = document.querySelectorAll(".menu-catalog__link");
            if (catalogLinks.length > 0) for (let link of catalogLinks) link.addEventListener("focus", (() => {
                const subActive = document.querySelector(".subcatalog__item_active");
                if (subActive) subActive.classList.remove("subcatalog__item_active");
            }));
            let catalogParentLinks = document.querySelectorAll(".menu-catalog__link-parent");
            if (catalogParentLinks.length > 0) {
                const mode = "subcatalog__item_active";
                for (let item of catalogParentLinks) item.addEventListener("keydown", (function(e) {
                    if (32 === e.keyCode) {
        ...
        let searcSelect = document.querySelector(".search-page__select");
        titleSearch.addEventListener("click", (() => searcSelect.classList.toggle("search-page__select_active")));
        let search__checkboxs = document.querySelectorAll(".categories-search__checkbox input");
        let searchValue = document.querySelector(".title-search__value");
        for (let checkbox of search__checkboxs) checkbox.addEventListener("click", (function() {
            let counter = document.querySelectorAll(".categories-search__checkbox input:checked").length;
            if (counter) searchValue.innerHTML = `Выбрано ${counter}`; else searchValue.textContent = "Везде";
        }));
        window["FLS"] = false;
        isWebp();
        addLoadedClass();
